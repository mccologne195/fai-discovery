...
- CPU model, disk size (`dmidecode`/`lsblk`), RAM size (`dmidecode`,
  with a fallback for VM firmware that reports DIMM size directly in
  GB instead of MB, e.g. Proxmox/EDK II)
